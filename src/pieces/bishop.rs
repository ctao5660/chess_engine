...
impl Piece for Bishop {
    fn get_moves(&self, b: &Board, position: i32) ->  Vec<[i32; 2]> {
        ...
        let mut curr_r = start_r + 1;
        let mut curr_c = start_c + 1;
        let mut valid_moves = Vec::new();
        while curr_r < 8 && curr_c < 8 { // Go up right diagonal
        ...
        curr_r = start_r - 1;
        curr_c = start_c - 1;
        while curr_r >= 0 && curr_c >= 0 { // down left
            if !self.add_valid_move(b, curr_r, curr_c, &mut valid_moves) {
                break;
            }
            curr_r -= 1;
        ...
        curr_r = start_r + 1;
        curr_c = start_c - 1;
        while curr_r < 8 && curr_c >= 0 { // up left
            if !self.add_valid_move(b, curr_r, curr_c, &mut valid_moves) {
                break;
            }
            curr_r += 1;
